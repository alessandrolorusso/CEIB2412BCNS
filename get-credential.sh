...
if [ -z $(gcloud vmware private-connections list --location $REGION --format="value(name)") ]
    then
        ...
        gcloud vmware private-connections create pc-connection-$REGION --location=$REGION --service-project=$service_project --type=PRIVATE_SERVICE_ACCESS --vmware-engine-network=$REGION-default || exit 1
    else
